...
def _round_report_markdown(round_result: dict) -> str:
    ...
    lines = [
        ...
        f"- Median Trade IRR: {summary['median_trade_irr']:.4f}" if summary.get("median_trade_irr") is not None else "- Median Trade IRR: N/A",
        f"- Expectancy: {summary['expectancy']:.4f}" if summary.get("expectancy") is not None else "- Expectancy: N/A",
        f"- MFE / MAE Ratio: {summary['mfe_mae_ratio']:.4f}" if summary.get("mfe_mae_ratio") is not None else "- MFE / MAE Ratio: N/A",
        f"- Median Days To Target: {summary['median_days_to_target']:.1f}" if summary.get("median_days_to_target") is not None else "- Median Days To Target: N/A",
        f"- Avg Stock CAGR (Legacy): {summary['avg_stock_cagr']:.4f}" if summary["avg_stock_cagr"] is not None else "- Avg Stock CAGR (Legacy): N/A",
        f"- Max Drawdown: {summary['max_drawdown']:.4f}",
        f"- Win Rate: {summary['win_rate']:.4f}" if summary["win_rate"] is not None else "- Win Rate: N/A",
        f"- Peak Gross Exposure Ratio: {summary['peak_gross_exposure_ratio']:.4f}" if summary.get("peak_gross_exposure_ratio") is not None else "- Peak Gross Exposure Ratio: N/A",
        f"- Used Cash Ratio: {summary['used_cash_ratio']:.4f}",
        f"- State Reject Rate: {summary['state_reject_rate']:.4f}" if summary.get("state_reject_rate") is not None else "- State Reject Rate: N/A",
        "",
        "## Exit Reasons",
    ]
