...
CAPACITY_PATTERNS = [
    re.compile(r"(?P<value>\d+(?:\.\d+)?)\s*万吨/年(?P<label>.{0,24}?(?:纯碱|金属钠|糊树脂|氯酸钠|PVC|液氯|烧碱))"),
    re.compile(r"(?P<label>(?:纯碱|金属钠|糊树脂|氯酸钠|PVC|液氯|烧碱).{0,24}?)(?P<value>\d+(?:\.\d+)?)\s*万吨/年"),
]

KEY_INPUT_KEYWORDS = ["原煤", "兰炭", "电力", "蒸汽", "石灰石", "天然碱", "原盐", "煤炭", "焦炭", "天然气"]
...
COST_STRUCTURE_RELATED_KEYWORDS = COST_STRUCTURE_CORE_KEYWORDS + (
    "纯碱",
    "烧碱",
    "PVC",
    "电力",
...
def _extract_cost_structure(scan_data: dict, field_hits: dict, docling_text: str) -> dict:
    ...
    return {
        ...
        "price_cost_comment": "纯碱等成本降幅远低于售价降幅" if "成本降幅远低于售价降幅" in combined_text else "",
    }
...
if __name__ == "__main__":
    ...
    scan_data = json.load(open(raw_path, "r", encoding="utf-8"))
    tier0_prep = json.load(open(tier0_path, "r", encoding="utf-8"))
    pdf_index_result = json.load(open(pdf_path, "r", encoding="utf-8"))
