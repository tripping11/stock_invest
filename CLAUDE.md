...
```powershell
python .agents/skills/market-opportunity-scanner/scripts/engines/radar_scan_engine.py A-share --limit 24
```

也可直接传代码列表：

```powershell
python .agents/skills/market-opportunity-scanner/scripts/engines/radar_scan_engine.py 600328,600348,600893
```

### 单票深挖

```powershell
python .agents/skills/single-stock-deep-dive/scripts/engines/deep_sniper_engine.py 600328 中盐化工
```

若只跑 Tier 1 与框架判断：

```powershell
python .agents/skills/single-stock-deep-dive/scripts/engines/deep_sniper_engine.py 600328 中盐化工 --skip-tier0
```
